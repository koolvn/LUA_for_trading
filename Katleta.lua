...
katleta = ol/GO												-- Макс. кол-во лот на всю катлету
DC="160402" 												-- Время дневного клиринга
EC="210002" 												-- Время вечернего клиринга
MC="120002"													-- Время утреннего клиринга

message("На всю катлету можно купить  "..katleta.." лотов по RIH6",1)
run=true
...
function OnClearing()
	if (os.date("%H%M%S")== DC or os.date("%H%M%S")== EC or os.date("%H%M%S")==MC) then
		message("На всю катлету можно купить  "..katleta.." лотов по RIH6",1)
		str="На всю катлету можно купить  "..katleta.." лотов по RIH6"
		log(str)
	end
end
