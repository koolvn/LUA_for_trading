...
function sendStopOther(class,security,direction,stopprice,dealprice,volume,account,exp_date,client_code,comment,class_other,sec_other)
	-- отправка простой стоп-заявки
	-- все параметры кроме кода клиента,коментария и времени жизни должны быть не нил
	-- если код клиента нил - подлставляем счет
	-- если время жизни не указано - то заявка "До Отмены"
	...
	--     1. ID присвоенный транзакции либо nil если транзакция отвергнута на уровне сервера Квик
	--     2. Ответное сообщение сервера Квик либо строку с параметрами транзакции
	if (class==nil or security==nil or direction==nil or stopprice==nil or volume==nil or account==nil or dealprice==nil or class_other==nil or sec_other==nil) then
		return nil,"QL.sendStop(): Can`t send order. Nil parameters."
	end

	--local pxabc = toPrice(sec_other,stopprice,class_other)
	local trans_id=random_max()
	local transaction={
		["TRANS_ID"]=tostring(trans_id),
		["ACTION"]="NEW_STOP_ORDER",
		...
		["OPERATION"]=direction,
		["QUANTITY"]=string_format("%d",tostring(volume)),
		["STOPPRICE"]=toPrice(sec_other,stopprice,class_other),
		["PRICE"]=toPrice(security,dealprice,class),
		["ACCOUNT"]=tostring(account),
		
		["STOP_ORDER_KIND"]="CONDITION_PRICE_BY_OTHER_SEC",
		["STOPPRICE_CLASSCODE"]=class_other,
		["STOPPRICE_SECCODE"]= sec_other,
		["STOPPRICE_CONDITION"]= condition
	}
	if direction == "B" then
		transaction["STOPPRICE_CONDITION"] = ">="
	else
		transaction["STOPPRICE_CONDITION"] = "<="
	end
	if client_code==nil then
